...
* It looks like you do not need the second lag, as the naive z-score and the robust z-score for lag 2 are respectively 0.49 and 0.55. Thus the model from here on now
...
lme1 <- mice::complete(imp, "all") %>% 
  purrr::map(lme4::glmer,
             formula = response ~ treatment + visit + (1 | id),
             family = binomial) %>% 
  pool()
  summary()
```
